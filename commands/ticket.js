...
function cleanDisplayName(name) {
  return name.replace(/[^ \wぁ-んァ-ヶ一-龥()（）ー・\-\_]/g, '');
}

function makeChannelName(display, username) {
  return `🎫｜${cleanDisplayName(display)}（${username}）`.slice(0, 100);
}
  ...
  async execute(interaction) {
    ...
    if (id.startsWith('ticket-open-')) {
      ...
      try {
        ...
        const name = makeChannelName(interaction.member.displayName, interaction.user.username);
    ...
    if (id.startsWith('ticket-close-')) {
      ...
      try {
        ...
        const guild = interaction.guild;
        const invite = await guild.invites.create(interaction.channel.parentId || guild.systemChannelId, {
          maxUses: 1,
          unique: true
        }).catch(() => null);
